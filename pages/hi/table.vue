...
function select(row: RowData): void {
...
const search = ref('')
const selectedStatus = ref<TodkStatus[]>([])
...
// Data
const { data: todos, pending } = await useLazyAsyncData('todos', () => $fetch<RowData[]>(`https://jsonplaceholder.typicode.com/todos${searchStatus.value}`, {
  query: {
    q: search.value,
    _page: page.value,
